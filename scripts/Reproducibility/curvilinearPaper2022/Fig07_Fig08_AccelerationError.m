%==========================================================================
% Script used to create Figure 7 and 8
...
% meshes
numFacesCoarsest  =  2100;  % coarsest test mesh
numFacesFinest    = 23000;  % finest test mesh
...
% set high resolution as the truth model
...
meshCoarse.setNumFaces(numFacesFinest);   % coarsen to

% generate one of each type of gravity model we are testing for
% sequentially coarser meshes down to 1050 vertices.
i=1;

while meshCoarse.numFaces>numFacesCoarsest

    % get coarse mesh
    meshCoarse.coarsen(meshCoarse.numFaces-1000);
    meshCoarse.smooth(1);
    meshCoarse.projectOnTo(mesh);

    % create analytic polyhedral model
    res(i) = meshCoarse.resolution;
    numFacesFinest(i) = meshCoarse.numFaces;
    polyhedralModel{i} = AnalyticPolyhedralModel(meshCoarse,Mu);

    % create quadrature models on curvilinear meshes
    for j = 1:length(quadRules)

        meshCoarse.setDegree(quadRules(j));  
        meshCoarse.curve(mesh);

        quadratureModel{i}{j} = ApproximatePolyhedralModel(meshCoarse,Mu);
        numQuadPoints(i,j) = quadratureModel{i}{j}.numElements;
    end
    i=i+1;
end

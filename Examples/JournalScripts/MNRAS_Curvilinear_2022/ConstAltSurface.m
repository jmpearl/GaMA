...
meshOriginal = mesh;
i=1;
quadRules = {'L1','L2','L3','G1','G2','B2','B3','O4'};
...
while mesh.numVertices>2000
    ...
    for j = 1:length(quadRules)
        quadratureModel{i}{j} = ApproximatePolyhedralModel(mesh,Mu,quadRules{j});
    end
    i=i+1
end
...
for i = 1:length(altitudes)
    i
    constAltSurface = meshCoarse.offsetSurfaceMesh(altitudes(i), 200);
    ...
    for k = 1:length(polyhedralModel)
        ...
        error(iter,1) = mean(vecnorm((accTempPoly - accOriginal),2,2)./accOgMag);
